...
def write_terraform_config(storage_account_name, storage_container_name, storage_account_key,
    ...
    terraform_lines = [
        ...
        '}',
        'provider "azurerm" {',
        '  alias           = "shm_management"',
        '  version         = "=1.35"',
        '  subscription_id = "{}"'.format(management_subscription_id),
        '}',
        'provider "null" {',
        '  version = "=2.1"',
        '}',
...
def write_terraform_variables(config, tenant_id, management_subscription_id):
...
def main():
    ...
    credentials = authenticate_device_code(tenant=args.tenant)
    location = config["dsg"]["location"]
    storage_container_name = "terraformbackend"

    # Get subscription details
    subscription, _ = get_subscription(config["dsg"]["subscriptionName"], credentials)
    management_subscription, _ = get_subscription(config["shm"]["subscriptionName"], credentials)
    ...
    write_terraform_config(storage_account_name, storage_container_name, storage_account_key,
                           subscription.subscription_id, management_subscription.subscription_id)
