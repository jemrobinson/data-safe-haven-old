...
resource "azurerm_virtual_network" "dsgroup9_vnet1" {
  name                = "TERRAFORM_DSGROUP9_VNET1"
...
resource "azurerm_subnet" "gateway" {
  name                 = "GatewaySubnet"
  resource_group_name  = "${azurerm_resource_group.networks.name}"
  virtual_network_name = "${azurerm_virtual_network.dsgroup9_vnet1.name}"
  address_prefix       = "10.10.1.0/24"
}

# Create a public IP for the gateway
resource "azurerm_public_ip" "gateway" {
  name                = "TERRAFORM_VNET1_GW_PIP"
  location            = "${azurerm_resource_group.networks.location}"
  resource_group_name = "${azurerm_resource_group.networks.name}"
